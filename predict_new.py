...
class CarlaMultiTaskModelTester:
    ...
    def __init__(
        ...
        self.spawn_point_index_in_use = None
        self.spawn_location_in_use = None
        ...
        if self.enable_mask_visualization:
            try:
                ...
                print("Continue without attention heatmap visualization")
        ...
        # 3) 会话级历史记录（用于结束时统计摘要）。
        self.history = []
        self.ability_metrics = AbilityMetricsTracker(
            csv_path=Path(__file__).with_name("ability_test.csv")
        )
    ...
    def spawn_vehicle(self, spawn_index=0):
        ...
        try:
            ...
            self.spawn_point_index_in_use = int(spawn_index)
            self.spawn_location_in_use = f"spawn_point[{self.spawn_point_index_in_use}]"
    ...
    def collision_callback(self, event):
        if self.ability_metrics is not None:
            other_actor = getattr(event, "other_actor", None)
            other_actor_type = getattr(other_actor, "type_id", None)
            self.ability_metrics.record_collision(other_actor_type=other_actor_type)

    # 压线传感器回调：上报一次压线事件。
    def lane_invasion_callback(self, event):
        if self.ability_metrics is not None:
            self.ability_metrics.record_lane_invasion()

    # 生成碰撞与压线事件传感器用于评测统计。
    def spawn_event_sensors(self):
        # 生成碰撞与压线传感器，用于能力评测指标统计。
        bp_lib = self.world.get_blueprint_library()
        ...
        try:
            ...
            self.collision_sensor = None
            print(f"Warning: failed to spawn collision sensor: {e}")
        ...
        try:
            ...
            self.lane_invasion_sensor = None
            print(f"Warning: failed to spawn lane invasion sensor: {e}")
    ...
    def _render_attention_heatmap_view(self, inference_bundle, current_speed_kmh, frame_count, width=480, height=360):
        ...
        if not self.enable_mask_visualization or self.mask_visualizer is None:
            ...
            cv2.putText(view, "Attention heatmap disabled", (12, height // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (200, 200, 200), 1)
            return view
    ...
    def _apply_traffic_light_stop_safety(self, throttle, brake, tl_label, tl_prob, current_speed):
        ...
        if self.red_stop_active:
            ...
            target_brake = self.red_stop_min_brake + 0.7 * speed_ratio
    ...
    def run_test(
        ...
        spawn_location_text = self.spawn_location_in_use if self.spawn_location_in_use else "unknown"
        self.ability_metrics.start_run(
            weather_type=weather_type,
            spawn_location=spawn_location_text,
            world_map=self.map,
            run_start_wall_time=start_time,
        )
        frame_count = 0
        screenshot_count = 0
        ...
        try:
            while time.time() - start_time < duration:
                ...
                self.ability_metrics.update_step(
                    elapsed_s=elapsed,
            ...
            self.ability_metrics.finalize_and_report(run_end_wall_time=time.time())
